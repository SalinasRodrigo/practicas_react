...
export function App() {
  ...
  function checkWin(nextSquares) {
    ...
    for (j = 0; j <= 6; j += 3) {
      if (
        nextSquares[j] == nextSquares[j + 1] &&
        nextSquares[j] == nextSquares[j + 2] &&
        nextSquares[j] != null
      ) {
    ...
    for (j = 0; j <= 2; j++) {
      if (
        nextSquares[j] == nextSquares[j + 3] &&
        nextSquares[j] == nextSquares[j + 6] &&
        nextSquares[j] != null
      ) {
        setWinState(nextSquares[j]);
        return nextSquares[j];
    ...
    if (
      ((nextSquares[0] == nextSquares[4] && nextSquares[0] == nextSquares[8]) ||
        (nextSquares[2] == nextSquares[4] &&
          nextSquares[2] == nextSquares[6])) &&
      nextSquares[4] != null
    ) {
